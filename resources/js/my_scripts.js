...
*/
			function loadStatsPage(){
				var tb = document.getElementById('stats_table');
				var win = 0;
				var lose = 0;
				for(var i = 2; i<tb.rows.length; i++){
					var home = parseInt(tb.rows[i].cells[2].innerHTML);
					var opp = parseInt(tb.rows[i].cells[3].innerHTML);
					if(home > opp){
						var winner = "CU";
						win++;
					}else if(home < opp){
						var winner = tb.rows[i].cells[1].innerHTML;
						lose++;
					}else{
						var winner = "Tie";
					}
					tb.rows[i].cells[4].innerHTML = winner;
				}
...
*/
		...
		function loadPlayersPage(){
			var selection = document.getElementById("player_selector").innerHTML;
